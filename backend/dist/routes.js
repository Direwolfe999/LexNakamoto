...
router.get("/api/sponsor/policy", async (req, res) => {
    try {
...
router.post("/api/sponsor/policy/check", async (req, res) => {
    try {
        const principal = req.body?.principal ? String(req.body.principal) : undefined;
...
router.get("/api/escrow/status/:txid", async (req, res) => {
    try {
...
router.post("/api/indexer/watch", async (req, res) => {
    try {
...
router.get("/api/indexer/tx/:txid", async (req, res) => {
    try {
        const txid = Array.isArray(req.params.txid) ? req.params.txid[0] : req.params.txid;
...
router.get("/api/indexer/watched", (_req, res) => {
...
router.post("/api/disputes/:id/evidence", async (req, res) => {
    try {
        ...
        if (!content || !submittedBy)
            return res.status(400).json({ error: "content and submittedBy are required" });
...
router.patch("/api/disputes/:id/status", async (req, res) => {
    try {
        const disputeId = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
        ...
        const dispute = disputeService.setStatus(disputeId, statusRaw, note);
...
router.get("/api/disputes", (_req, res) => {
    return res.json({ ok: true, items: disputeService.listDisputes() });
});
router.get("/api/disputes/:id", (req, res) => {
    const disputeId = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
...
router.post("/api/notifications/subscribers", (req, res) => {
...
router.get("/api/notifications/subscribers", (_req, res) => {
...
router.get("/api/notifications/events", (_req, res) => {
