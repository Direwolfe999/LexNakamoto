...
export type SponsorPolicySummary = {
  rules: {
...
export type SponsorPolicyDecision = {
  eligible: boolean;
  reasons: string[];
  principal?: string;
};
...
export class SponsorService {
  ...
  private readonly maxEscrowSats: bigint;
  private readonly allowedTokenContracts: Set<string>;
  private readonly minSponsorBalanceMicroStx: bigint;
  private readonly lowGasWarningMicroStx: bigint;
  private readonly cooldownSeconds: number;
  private readonly lastSponsoredByPrincipal = new Map<string, number>();
  ...
  constructor() {
    ...
    if (!this.sponsorPrivateKey) throw new Error("SPONSOR_PRIVATE_KEY is required");
    if (!this.contractAddress) throw new Error("ESCROW_CONTRACT_ADDRESS is required");

    this.sponsorAddress = getAddressFromPrivateKey(this.sponsorPrivateKey, "testnet");
  }

  async sponsorAndBroadcast(input: SponsorInput): Promise<SponsorResult> {
  ...
  async getPolicySummary(): Promise<SponsorPolicySummary> {
  ...
  async evaluatePolicy(input: {
    ...
    if (input.principal) {
      ...
      if (cooldownMessage) reasons.push(cooldownMessage);
  ...
  private deserialize(txHex: string): StacksTransactionWire {
    const clean = txHex.startsWith("0x") ? txHex.slice(2) : txHex;
    return deserializeTransaction(clean);
  }

  private extractContractCallDetails(tx: StacksTransactionWire) {
    ...
    const contractAddress =
      payload?.contractAddress?.address ?? payload?.contractAddress?.toString?.();
  ...
  private enforceFunctionPolicies(functionName: string, functionArgs: any[]) {
    if (functionName !== "create-escrow") return;
    ...
    if (escrowAmount > this.maxEscrowSats) {
      throw new Error(
        `Escrow amount exceeds sponsor policy limit (${escrowAmount} > ${this.maxEscrowSats} sats)`
      );
    }
  ...
  private enforceRateLimit(ip: string, principal?: string) {
    const now = Date.now();
    const keys = [
      `ip:${ip || "unknown"}`,
  ...
  private enforceCooldown(principal?: string) {
    if (!principal) return;
    const now = Date.now();
  ...
  private getCooldownFailureReason(principal: string, now: number): string | null {
  ...
  private async ensureSponsorGasSafety() {
    const balance = await this.getSponsorStxBalance();

    if (balance < this.lowGasWarningMicroStx) {
      console.warn(
        `[Low Gas Warning] sponsor=${this.sponsorAddress} balance=${this.microStxToStx(balance)} STX. Top up from faucet soon.`
      );
    }

    if (balance < this.minSponsorBalanceMicroStx) {
      throw new Error(
        `Sponsor wallet balance too low (${this.microStxToStx(balance)} STX). Minimum required: ${this.microStxToStx(this.minSponsorBalanceMicroStx)} STX`
      );
    }
  }

  private async getSponsorStxBalance(): Promise<bigint> {
  ...
  private extractUint(cv: any): bigint | null {
    if (!cv) return null;
    if (typeof cv.value === "bigint") return cv.value;
    if (typeof cv.value === "number") return BigInt(cv.value);
    if (typeof cv.value === "string") return BigInt(cv.value);
    return null;
  }

  private extractContractPrincipal(cv: any): string | null {
    if (!cv) return null;
